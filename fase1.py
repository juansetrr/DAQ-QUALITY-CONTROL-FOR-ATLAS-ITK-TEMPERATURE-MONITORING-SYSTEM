...
voltajes_opc = [None] * 48   # Lista para almacenar los voltajes OPC por canal
datos_opc_actualizados = [False] * 48  # Lista para rastrear si los datos OPC han sido actualizados
    ...
    def datachange_notification(self, node, val, data):
        try:
            ...
            if match:
                ...
                if canal < 48:  # Solo procesar hasta el canal 48
                    valor_en_volts = val 
...
# Función actualizada para convertir temperatura a voltaje (no lineal)
def convertir_celsius_a_voltaje(T):
    ...
    if discriminante < 0:
        raise ValueError(f"Discriminante negativo para T={T}. No hay solución real para R.")
    else:
        sqrt_disc = math.sqrt(discriminante)
        # Tomamos la raíz positiva
        R = (-b + sqrt_disc) / (2 * a)
        if R <= 0:
            raise ValueError(f"La resistencia R calculada es negativa o cero para T={T}.")
...
for temp in lista_temperaturas:
    try:
        volt = convertir_celsius_a_voltaje(temp)
        lista_voltajes.append(volt)
    except ValueError as e:
        print(f"No se pudo calcular el voltaje para T={temp}: {e}")
        sys.exit(1)
...
for temp, valor_fuente in zip(lista_temperaturas, lista_voltajes):
    # Ajustar el voltaje en la fuente
    fuente.write(f"VOLT {valor_fuente}")
    print(f"Ajustando voltaje de la fuente a {valor_fuente:.6f} V para T={temp}°C")
    ...
    #-----------
    print(f"Esperando datos OPC para el voltaje de {valor_fuente:.6f} V...")

    timestamp = datetime.datetime.now().strftime("%d-%H:%M:%S.%f")[:-3]  # %f da los microsegundos, y luego se recorta a milisegundos

    print(f"{timestamp}")
    # Esperar a que se reciban datos nuevos de los 48 canales OPC
    ...
    print(f"{timestamp} - Temperatura objetivo: {temp}°C, Voltaje de la fuente: {valor_fuente:.6f} V, Voltaje medido: {valor_medido} V")
